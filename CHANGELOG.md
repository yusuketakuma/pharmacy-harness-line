...
## Pharmacy v0.26.0 (2026-08-18)

### 本番バージョン表示の修正

- Cloudflareの直接デプロイでも、Workerへリリースバージョン・ビルド時刻・成果物ハッシュを注入
- 本番の`/admin/version`が`0.26.0`を返すようにし、管理画面の更新表示と実際の配信バージョンを一致
- Worker、Admin、薬局LIFF、Worker Assetsをビルドした後にバージョン情報を注入し、Workerを再ビルドしてからデプロイ
- デプロイ後にWorkerが期待したバージョンを返すことを自動確認し、古い`0.0.0-dev`メタデータのまま公開される状態を検出
- 管理画面の処方せん画像取得から不要な`Cache-Control`リクエストヘッダーを除去し、クロスオリジンのプリフライト失敗で画像を表示できない問題を修正
- 管理画面の配信作成とLIFFの予約・イベント予約で使用する`Idempotency-Key`をCORS許可リストへ追加し、別オリジン構成で送信がプリフライト拒否される問題を修正
- 既存のD1・R2・Secrets・LINE設定・患者データ・リッチメニュー定義は更新対象から除外し、従来の保全ガードを継続

### 更新時の注意

- 配信バージョンの可観測性と、管理画面の処方せん画像表示を修正するリリースです
- 本番反映前に、デプロイ後の`/admin/version`確認と管理画面の更新表示を確認してください
