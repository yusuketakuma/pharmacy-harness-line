...
const patientLiffPageDefinitions: EntryDefinition[] = [
  ...
  {
    id: 'page-patient-liff-timeline', kind: 'page', surface: 'patient-liff', path: '/pharmacy/timeline',
    source: 'apps/liff/src/App.tsx', componentSource: 'apps/liff/src/custom/pharmacy/timeline/PatientTimelinePage.tsx',
    testReferences: ['apps/liff/src/custom/pharmacy/timeline/PatientTimelinePage.test.tsx', 'apps/liff/src/custom/pharmacy/request.test.ts'],
    roles: ['line-patient'], authority: 'verified LINE subject plus server owner/account scope across the existing patient domains',
    lineAccountIdAuthority: 'liffId is a selector; the server resolves the account and binds the verified LINE owner',
    displayedInfo: ['PHI-minimized owned timeline status and fixed next-action destinations'], mutation: 'read-only',
    confirmation: 'owner/account binding, allowlisted projection, fixed destinations, and private no-store response',
    phiClassification: 'PHI-free-default', audit: 'no patient identifier, medicine detail, staff note, or encrypted payload is returned',
    queryAuthority: 'selector-only-server-validated', manualOneToOne: 'not-applicable', meetFollowUp: 'not-applicable', reachability: 'reachable',
  },
  {
...
const apiDefinitions: EntryDefinition[] = [
  ...
  {
    id: 'api-pharmacy-beta-memberships',
    kind: 'api', surface: 'pharmacy-admin', path: `${customRoutePrefix}beta-memberships*`, source: 'apps/worker/src/custom/pharmacy/beta-membership/routes.ts',
    testReferences: ['apps/worker/src/custom/pharmacy/beta-membership/routes.test.ts'],
    roles: ['tenant-admin', 'staff'],
    authority: 'server pharmacy owner/admin staff and patient/account membership scope',
    lineAccountIdAuthority: 'server-derived and tenant/account validated; patient and membership ids are selectors only',
    displayedInfo: ['beta membership status and validity period'],
    mutation: 'grant/suspend/resume/revoke beta membership',
    confirmation: 'CSRF, expectedVersion, state transition and same-tenant patient validation',
    phiClassification: 'PHI',
    audit: 'membership grant/transition actor and state are tenant-audited atomically',
    queryAuthority: 'selector-only-server-validated', manualOneToOne: 'not-applicable', meetFollowUp: 'not-applicable', reachability: 'reachable',
  },
  {
    id: 'api-pharmacy-chat-templates',
    kind: 'api', surface: 'pharmacy-admin', path: `${customRoutePrefix}chat-templates*`, source: 'apps/worker/src/custom/pharmacy/chat-templates/routes.ts',
    testReferences: ['apps/worker/src/custom/pharmacy/chat-templates/routes.test.ts'],
    roles: ['tenant-admin', 'staff'],
    authority: 'server pharmacy staff/account scope; approval requires owner/admin role',
    lineAccountIdAuthority: 'server-derived and staff/account validated',
    displayedInfo: ['chat canned template list/detail'],
    mutation: 'create/update/archive templates; owner/admin approval required',
    confirmation: 'CSRF, expectedVersion CAS, PHI-free approved-message text validation, no self-approval',
    phiClassification: 'metadata-only-phi-free',
    audit: 'template lifecycle transitions are tenant-audited atomically',
    queryAuthority: 'selector-only-server-validated', manualOneToOne: 'not-applicable', meetFollowUp: 'not-applicable', reachability: 'reachable',
  },
  {
...
const patientLiffApiDefinitions: EntryDefinition[] = [
  ...
  {
    id: 'api-patient-liff-timeline', kind: 'api', surface: 'patient-liff', path: '/api/liff/pharmacy/timeline', source: 'apps/worker/src/custom/pharmacy/patient-timeline/routes.ts',
    testReferences: ['apps/worker/src/custom/pharmacy/patient-timeline/routes.test.ts', 'apps/worker/src/custom/pharmacy/patient-timeline/boundary.test.ts'],
    roles: ['line-patient'], authority: 'verified LINE subject and server-resolved owner/account scope across existing patient records',
    lineAccountIdAuthority: 'liffId is a selector; query account/friend ids are never authority and the owner binding is server resolved',
    displayedInfo: ['PHI-minimized owned timeline status and fixed next-action destinations'], mutation: 'read-only',
    confirmation: 'owner/account binding, bounded allowlisted projection, fixed destinations, and private no-store response',
    phiClassification: 'PHI-free-default', audit: 'no patient identifier, medicine detail, staff note, or encrypted payload is returned',
    queryAuthority: 'selector-only-server-validated', manualOneToOne: 'not-applicable', meetFollowUp: 'not-applicable', reachability: 'reachable',
  },
  {
