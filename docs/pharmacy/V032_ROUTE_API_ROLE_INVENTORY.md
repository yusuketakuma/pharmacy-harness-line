...
| `growth-loop/routes.ts` | `GET /api/custom/pharmacy/growth/config`; `PUT .../growth/config`; `GET /api/custom/pharmacy/readiness`; `GET .../active-work`; `GET .../operations-summary`; `GET .../action-queue`; `GET .../growth/dashboard`; `GET/POST .../growth/sources`; `PATCH .../growth/sources/:sourceId`; `POST .../growth/submissions/:submissionId/source`; `PUT .../growth/submissions/:submissionId/validity` | staff/capability/account scope; action queueは50件上限の非PHI/read-only、他のconfig/source/submission mutationはCSRF/revision/state / operational-sensitive; `growth-loop/routes.test.ts` / `action-queue.test.ts` |
