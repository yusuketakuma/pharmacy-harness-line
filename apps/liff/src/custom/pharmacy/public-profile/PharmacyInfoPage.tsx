...
function InfoLine({ label, value, className = 'mt-3 whitespace-pre-line text-base leading-6 text-gray-700' }: {
  label?: string;
  value: string;
  className?: string;
}) {
...
export function PharmacyInfoContent({ profile }: { profile: PharmacyPublicProfile }) {
  ...
  return (
    <div className="space-y-4 p-4">
      <section className="pharmacy-card p-5">
        <p className="text-sm font-bold tracking-wide text-green-800">PHARMACY</p>
        <h2 className="mt-1 text-2xl font-bold text-gray-950">{profile.display_name}</h2>
        {profile.closure_notice && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-base leading-6 text-amber-900"><span className="font-bold">休業・臨時案内</span><br />{profile.closure_notice}</p>}
      </section>
      <section className="pharmacy-card p-5" aria-labelledby="hours-title">
        <h2 id="hours-title" className="font-bold text-gray-950">営業時間</h2>
        <InfoLine value={profile.business_hours || '営業時間は薬局へお問い合わせください。'} className="mt-2 whitespace-pre-line text-base leading-7 text-gray-700" />
        <InfoLine label="処方せん受付時間" value={profile.prescription_reception_hours} />
        <InfoLine label="時間外の対応" value={profile.after_hours_note} />
      </section>
      {(profile.services_note || profile.supported_languages || profile.payment_methods) && <section className="pharmacy-card p-5" aria-labelledby="services-title">
      ...
      <section className="pharmacy-card p-5" aria-labelledby="access-title">
        <h2 id="access-title" className="font-bold text-gray-950">住所・アクセス</h2>
        <p className="mt-2 text-base leading-6 text-gray-700"><span className="font-medium">住所</span><br />{[profile.postal_code && `〒${profile.postal_code}`, profile.address].filter(Boolean).join(' ') || '未設定'}</p>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          ...
          {phoneHref && <a href={phoneHref} className="min-h-11 rounded-xl border border-green-700 bg-white px-4 py-3 text-center font-bold text-green-800">電話する</a>}
        </div>
      </section>
      {(websiteUrl || profile.updated_at) && <section className="pharmacy-card p-5">
        {websiteUrl && <a href={websiteUrl} target="_blank" rel="noreferrer noopener" className="pharmacy-control block rounded-xl border border-green-700 bg-white px-4 py-3 text-center font-bold text-green-800">公式サイト</a>}
        {profile.updated_at && <p className="mt-3 text-right text-sm text-gray-600">最終更新：{profile.updated_at.slice(0, 10)}</p>}
      </section>}
    </div>
  );
}
