...
describe('admin cookie serialization', () => {
  it('keeps tenant cookies site-wide with the existing attributes', () => {
    expect(adminSessionCookie('token/value', 'Lax')).toBe(
      'lh_admin_session=token%2Fvalue; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800',
    );
    expect(csrfCookie('csrf', 'Strict')).toBe(
      'lh_csrf=csrf; Path=/; Secure; SameSite=Strict; Max-Age=604800',
    );
  ...
  it('keeps platform-admin cookies scoped to their API prefix', () => {
    expect(platformAdminSessionCookie('token', 'Lax')).toBe(
      'lh_platform_admin_session=token; Path=/api/platform-admin; HttpOnly; Secure; SameSite=Lax; Max-Age=604800',
    );
    expect(platformAdminCsrfCookie('csrf', 'None')).toBe(
      'lh_platform_admin_csrf=csrf; Path=/api/platform-admin; Secure; SameSite=None; Max-Age=604800',
    );
  });
});
