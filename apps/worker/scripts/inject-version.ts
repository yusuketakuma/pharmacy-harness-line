...
function write(outPath: string, v: VersionData): void {
  ...
  for (const [field, value] of [
    ['version', v.version],
    ['workerPackageVersion', v.workerPackageVersion],
    ['webPackageVersion', v.webPackageVersion],
    ['liffPackageVersion', v.liffPackageVersion],
  ] as const) {
    if (!semver.test(value)) throw new Error(`inject-version: ${field} must be a semantic version`);
  }
  const body = [
