...
describe('pharmacy patient feature settings', () => {
  ...
  it('uses the existing account-scoped config endpoint with CAS and exposes one pharmacy route', () => {
    ...
    const growthApi = read('custom/pharmacy/growth-loop/api.ts')
    const richMenuApi = read('custom/pharmacy/rich-menu/api.ts')
    const genericApi = read('lib/api.ts')
    ...
    expect(page).toContain('一般スタッフは閲覧のみです。変更はオーナーまたは管理者が行ってください。')
    expect(page).toContain('disabled={!canMutate || !dirty')
    ...
    expect(page).not.toContain('pharmacyRichMenuApi.createPharmacyVersion')
    expect(page).not.toContain('initialMode="set-default"')
    expect(growthApi).toContain('saveConfig: (accountId: string')
    expect(growthApi).toContain('readiness: (accountId: string')
    expect(growthApi).toContain('configurationDoctor: {')
    expect(growthApi).toContain('activeWork: (accountId: string')
    expect(richMenuApi).toContain('pharmacyCandidate: (accountId: string)')
    expect(richMenuApi).toContain('/api/custom/pharmacy/rich-menus/candidate?accountId=')
    expect(richMenuApi).toContain('/api/custom/pharmacy/rich-menus/candidate/image?')
    expect(genericApi).not.toContain('/api/custom/pharmacy/')
