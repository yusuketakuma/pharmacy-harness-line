...
describe('emergency contraception admin API', () => {
  ...
  it('scopes config and queue reads to the selected LINE account', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ intakes: [] })))
    ...
    expect(fetchMock.mock.calls[1][0]).toBe(
      'https://worker.example/api/custom/pharmacy/emergency-contraception/intakes?line_account_id=account%2F1',
    )
  })
  ...
  it('uses the Worker route contract for staff mutations', async () => {
    ...
    expect(fetchMock.mock.calls.map(([url, init]) => [url, init?.method, init?.body])).toEqual([
      ['https://worker.example/api/custom/pharmacy/emergency-contraception/config?line_account_id=account-1', 'PUT', JSON.stringify({ enabled: false, purposeText: '対面相談受付' })],
