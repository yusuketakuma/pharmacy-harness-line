...
const emptyDraft: PharmacyPublicProfileInput = {
  displayName: '', phone: '', faxNumber: '', postalCode: '', address: '', businessHours: '',
...
const profileFields: Array<{
  key: keyof PharmacyPublicProfileInput
  label: string
  rows?: number
  maxLength: number
  placeholder: string
  ...
  { key: 'phone', label: '電話番号', maxLength: 40, placeholder: '例：03-1234-5678', type: 'tel' },
  { key: 'faxNumber', label: 'FAX番号', maxLength: 40, placeholder: '例：03-1234-5679', type: 'tel' },
  ...
  { key: 'googleMapsUrl', label: 'Google Maps URL', maxLength: 2000, placeholder: '未入力なら住所から検索リンクを作成します', type: 'url' },
  { key: 'websiteUrl', label: '公式サイトURL', maxLength: 2000, placeholder: '例：https://example-pharmacy.jp/', type: 'url' },
]
...
function draftFromProfile(profile: PharmacyPublicProfile | null): PharmacyPublicProfileInput {
  ...
  return {
    displayName: profile.display_name, phone: profile.phone, faxNumber: profile.fax_number,
