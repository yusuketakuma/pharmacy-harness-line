...
describe('cross-screen safety and accessibility', () => {
  ...
  it('fails closed when authenticated browser session state cannot be stored', () => {
    const authGuard = source('../components/auth-guard.tsx')
    const login = source('./login/page.tsx')
    const api = source('../lib/api.ts')
    expect(authGuard).toContain('setSessionSafetyError(')
    expect(authGuard).toContain('persistStaffSession(data)')
    expect(login).toContain('persistStaffSession(loginData)')
    expect(login).toContain('const csrfToken = getCsrfToken()')
    expect(api).toContain('if (!isCsrfToken(token)) throw new SessionStateUnavailableError')
    const responseIndex = authGuard.indexOf('const data = await res.json()')
    const cancelledIndex = authGuard.indexOf('if (cancelled) return', responseIndex)
    const persistIndex = authGuard.indexOf('persistStaffSession(data)')
    expect(responseIndex).toBeGreaterThanOrEqual(0)
    expect(cancelledIndex).toBeGreaterThan(responseIndex)
    expect(cancelledIndex).toBeLessThan(persistIndex)
  })
})
