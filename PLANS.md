...
**初回cohort案**: 1薬局・最大5名、成人本人利用を基本とする。氏名等は本台帳に記載しない。人数・参加資格・family/EC/Mynaの有効範囲は公開前に責任者が確定する。初回参加対象を絞ることは既存API/機能の削除・既存案件の放棄を許可しない。
...
- V035-0/4/5/6: [readiness evidence](docs/pharmacy/evidence/v0.35.0-beta-staff-readiness.json)へ7業務、旧Human Gate全25行、残項目・安全gate66項目（任意追加画面3項目を含む）の本文/子要件の引継ぎ先、実スタッフ6課題×各3試行、未決の運用条件を結合した。inventoryの参照漏れ検査はPASSだが、担当者の受諾・適用scope承認・exact候補・実スタッフ試験・参加者境界実装・運用条件確定は未完了。本人のみ/代理患者を含むか、曜日/時間帯/一次応答期限/主担当/代行担当を確認中。復旧目標は既存V032-5のRPO24時間/RTO4時間/最低3世代を引き継ぐ。packageは`0.34.2`のまま、commit/push/deploy/本番変更/activationは実行していない。
